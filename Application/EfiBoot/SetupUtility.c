  ...
  )
{
  ...
  BlockH = (UINTN)LOGO_HEIGHT + BOOT_WAIT_TEXT_GAP + GFX_FONT_SIZE;
  LogoX  = (W > (UINTN)LOGO_WIDTH) ? ((W - (UINTN)LOGO_WIDTH) / 2) : 0;
  LogoY  = (H > BlockH) ? ((H - BlockH) / 2) : 0;
  TextY  = LogoY + (UINTN)LOGO_HEIGHT + BOOT_WAIT_TEXT_GAP;

  GfxDrawLogo (LogoX, LogoY);
  GfxDrawTextCentered (TextY, &mClrMuted, &mClrBg, STR_BOOT_WAIT_HINT);
}
